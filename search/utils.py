...
def eigendecompose_laplacian(
    ...
    if normalized_laplacian:
        ...
        if normalized_eigenvalues:
            ...
            L /= 2. # divide by 2 to ensure the eigenvalues are between [0, 1]
    else:
...
def prune_baseline(X_queried: torch.Tensor,
                   Y_queried: torch.Tensor,
                   index_to_keep: torch.Tensor):
    # I first unsqueeze & reshape X_queried from (n, k) to (n, 1, k) then expand the 
    # second dimension to the number of rows in index_to_keep
    X_q_exp = X_queried.unsqueeze(-1).reshape(-1,1,
                X_queried.shape[-1]).expand(-1,index_to_keep.shape[0],-1)
    # next I compare if the corresponding element are the same and 
    ...
    return X_queried[train_idx], Y_queried[train_idx] # corresponding to X_train, Y_train
...
def Index_Mapper(Graph):  # e.g. (2,8,13)-> 3, we need numerical indices for kernel computation
    ...
    mapping = {tuple_node: int_node for int_node, tuple_node in enumerate(node_list, start=0)}
    remapping = {v: k for k, v in mapping.items()}
    return mapping, remapping

def ComboNeighbors_Generator(underlying_graph: nx.Graph, # the original underlying graph 
    ...
    G.add_edges_from(NeighborFinder(underlying_graph, X)) # get the 1-hop combo-neighbors
...
def generate_start_location(underlying_graph: nx.Graph, # the original graph
                            k: int, # number of elements in the combination
                            method: str,): # the method we use to generate initial location
    if method == "ei":
