...
def restart(
    ...
    default_options = {"n_nodes_min": 5, "fail_tol": 20,"succ_tol": 10, 
    ...
    if anchor is not None: # use the specified restart location
        ...
        while len(candidates) < n_init:
            candidate_seeds = torch.from_numpy(np.random.RandomState(
                seed*patience+iter).choice(2**32,n_init)) # generate random seeds first
            candidates = candidates if len(candidates) else torch.zeros([0,k])
            for seed_ in candidate_seeds:
                if initialization: # use the same starting location at initialization for all methods 
                    candidate = torch.from_numpy(np.random.RandomState(
                        seed_).choice(list(base_graph.nodes),k,replace=False))
                ...
                candidate = torch.sort(candidate)[0] # Important! sort the node label for unique indentification
            ...
            if iter >= patience:
                raise RuntimeError("Can not sample enough combo-nodes at restart with the current patience")
            iter+=1
