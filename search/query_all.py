...
def run_search(
    ...
    candidate_neighbours = ComboNeighbors_Generator(Problem.underlying_graph, candidate_start.squeeze()) # track the degree
    n_init_actual = min(n_initial_points, len(candidate_neighbours))
    candidates_idx = torch.from_numpy(np.random.RandomState(seed).choice(np.arange(candidate_neighbours.shape[0]),
                                                                         n_initial_points,replace=False))
    candidates = torch.cat([candidate_start, candidate_neighbours[candidates_idx]], dim=0)
    ...
    while len(X_queried) < iterations:
        start_time = time()
        anchor = queried_best_loc.unsqueeze(0) if restart_location_method=="queried_best" else start_location
        n_initial_points = 1
        ...
        if label == "random": # We use a random sampler similar to "restart"
            ...
            candidates, ComboGraph = Local_Search(Problem,
                                      X_queried, 
                                      queried_best_loc,
                                      seed,
                                      k,
        ...
        if new_y.shape[0] == 1 and not iter%1:
            if ground_truth is not None:
                ...
                current_regret = 0.0
            print(f'Seed{seed} iter:{iter} ' # + f'Candidate:{candidates.squeeze().tolist()}, ' 
                  f'obj:{new_y.item():.4f} '
