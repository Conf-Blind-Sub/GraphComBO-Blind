...
# ------------------------ BO-Set --------------------------
def GraphBO_Search(Problem: object, 
                   ...
                   tuple_to_int_mapper: dict, int_to_tuple_mapper: dict, # the local-global index remappers
                   cached_eigenbasis, use_cached_eigenbasis, # cached eigen-basis
                   trust_region_state, trust_region_kwargs, # trust-region state variable 
                   acqf_kwargs, acqf_optim_kwargs, # acqusition function settings
                   model_optim_kwargs):
    if not trust_region_state.restart_triggered: 
        ...
        model, mll, cached_eigenbasis = initialize_model(
            ...
            covar_kwargs = {"order": model_configurations["order"]}, ## No order means context graph size
        ...
        acq_func = get_acqf(model, X_baseline=X_mapped, train_Y=Y_train, 
                            batch_size=batch_size, 
                            acq_type=acqf_kwargs['type'])
        # 3. Optimise the Acquisition function
        raw_candidates = optimize_acqf(acq_func, context_graph=context_graph,
                                       method="enumerate", batch_size=batch_size,
                                       X_avoid=X_mapped, X_prior=X_prior,
                                       acq_type=acqf_kwargs['type'], 
                                       **acqf_optim_kwargs,)
        if raw_candidates is None: # Resart if all the nodes in the context subgraph have been queried
            trust_region_state.restart_triggered = True 
            candidates = None
            print("***** All ComboNodes in the current ComboSubgraph are queried! *****")
        else:
            # the candidates labels are in terms of the local graph -- map them back to the labels of global graph
            candidates = int_to_tuple_mapper(raw_candidates)
    else: # when a restart is triggered
        print(f"========== Failtol Reached & Restart Triggered ========")
        n_restart += 1
        anchor = None if converge else anchor # randomly respawn when all nodes in the subgraph are queried 
    ...
    return context_graph, X_train, Y_train, candidates, trust_region_state, n_restart, cached_eigenbasis
